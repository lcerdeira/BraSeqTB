//
// This file holds several functions specific to the workflow/braseqtb.nf in braseqtb
//
// Modified from NF-Core's template: https://github.com/nf-core/tools

class Workflowbraseqtb {

    //
    // Check and validate parameters
    //
    public static String initialise(workflow, params, log, schema_filename=['conf/schema/braseqtb.json']) {
        def Integer error = 0
        def String run_type = ""
        ...
        if (['braseqtb', 'staphopia'].contains(params.wf)) {
        ...
        if (Utils.isLocal(params.outdir)) {
            ...
            if (!workflow.resume) {
                ...
                new File("${params.outdir}/braseqtb-comparative/${params.wf}/${params.run_name}").eachDirRecurse { item ->
