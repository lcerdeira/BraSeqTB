//
// This file holds several functions specific to the workflow/braseqtbtools.nf in braseqtb
//
// Modified from NF-Core's template: https://github.com/nf-core/tools

class WorkflowbraseqtbTools {

    //
    // Check and validate parameters
    //
    public static String initialise(workflow, params, log, schema_filename=['conf/schema/braseqtbtools.json']) {
        def Integer error = 0
        ...
        if (params.braseqtb) {
            if (Utils.isLocal(params.braseqtb)) {
                error += Utils.fileNotFound(params.braseqtb, 'braseqtb', log)
                if (error > 0) {
                    missing_required += "--braseqtb"
                }
            }
        } else {
            missing_required += "--braseqtb"
