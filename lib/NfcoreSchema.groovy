...
class NfcoreSchema {
    ...
    public static String listWorkflows(workflow, params) {
        ...
        for (group in params.available_workflows.keySet()) {
            String group_name = group == 'braseqtb' ? 'braseqtb' : 'braseqtb Tools'
            ...
            if (group == 'braseqtb') {
                for (wf in params.available_workflows[group].sort()) {
                    ...
                    String wf_name = wf == 'braseqtb' ? 'braseqtb (default)' : wf
                    output += "  " +  wf_name.padRight(max_chars) + description_default.padRight(10) + '\n'
                }
                output += '\n'
            } else {
                output += 'braseqtb Tools can include multiple tools (Subworkflows) or a single tool (Modules).\n\n'
