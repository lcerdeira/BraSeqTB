/*
========================================================================================
    Nextflow Functions specific to braseqtb Tools
========================================================================================
/*

/*
========================================================================================
    Public: collect_samples(braseqtb_dir, extension, exclude_list, include_list)

    braseqtb_dir -> the path to braseqtb outputs
...
========================================================================================
*/
def collect_samples(braseqtb_dir, extension, include_list, exclude_list) {
    include_all = true
    inclusions = []
    exclusions = []
    IGNORE_LIST = ['.nextflow', 'braseqtb-info', 'braseqtb-tools', 'work', 'braseqtb-runs']
    if (include_list) {
    ...
    sample_list = []
    missing = []
    file("${braseqtb_dir}/").eachFile { item ->
        if( item.isDirectory() ) {
            sample = item.getName()
            if (!IGNORE_LIST.contains(sample)) {
                if (inclusions.contains(sample) || include_all) {
                    if (!exclusions.contains(sample)) {
                        if (_is_sample_dir(sample, braseqtb_dir)) {
                            sample = _collect_inputs(sample, braseqtb_dir, extension)
                            ...
                            log.info "${sample} does not appear to be a braseqtb sample, skipping..."
                        }
                    }
                }
            }
...
========================================================================================
    ...
    is a required output and all samples processed by braseqtb should have it.
