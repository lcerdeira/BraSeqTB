/*
========================================================================================
    Functions specific to braseqtb
========================================================================================
*/
...
def get_schemas() {
    ...
    if (params.wf == "cleanyerreads") {
        ...
        // Named workflow based of braseqtb
        schemas << 'conf/schema/braseqtb.json'
    } else {
        // braseqtb Tool
        schemas << 'conf/schema/braseqtb-tools.json'
    }
...
def print_efficiency(cpus) {
    ...
    if (['standard', 'docker', 'singularity'].contains(workflow.profile)) {
        ...
        log.info """
            ...
            of braseqtb. You can use the '-qs' parameter to alter the number of 
...
def is_available_workflow(wf) {
    if (params.available_workflows['braseqtb'].contains(wf)) {
        return true
    } else if (params.available_workflows['braseqtbtools']['subworkflows'].contains(wf)) {
        return true
    } else if (params.available_workflows['braseqtbtools']['modules'].contains(wf)) {
        return true
    } else {
        return false
    }
        ...
        if (final_output) {
            if (args.opts.btype == "main" || args.opts.btype == "tools" || args.opts.btype == "teton") {
                ...
                if (goto_base) {
                    ...
                    // my-sample/braseqtb-main/assembler
                    if (process_name == "bakta" || process_name == "prokka") {
                        // my-sample/braseqtb-main/<process_name>/<output>
                        final_output = "${args.prefix}/${special_subdir}/${args.opts.btype}/annotator/${final_output}"
                    } else {
                        // my-sample/braseqtb/main/<output>
                        // my-sample/braseqtb/tools/<output>
                        // my-sample/braseqtb/teton/<output>
                        final_output = "${args.prefix}/${special_subdir}/${args.opts.btype}/${final_output}"
                    }
                }
            } else {
                // $outdir/braseqtb-runs/$rundir
                // braseqtb-runs/<WORKFLOW>-YYYYMMDD-HHMMSS/pangenome/core-genome.aln.gz
                final_output = "braseqtb-runs/${args.rundir}/${final_output}"
            }
