...
def create_resistance_df(sample_res, method):
    ...
    # DR variants from the BRATB analysis. Added after LoFreq DR variants to give priority to BRATB frequencies.
    ...
    # Other variants from BRATB analysis.
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Analyse resistance output from BRATB Pipeline')
    ...
    for file_name in json_major_vars:
        ...
        with open(file_name) as json_file:
            samples[keys]['bratb'] = json.load(json_file)
...
#===============
    for patient, sample in tqdm(samples_df.iterrows(), total=samples_df.shape[0]):
        ...
        pt_df_bratb = create_resistance_df(sample_res['bratb'], 'XBS')
        ...
        pt_df = pd.concat([pt_df_bratb, pt_df_lof, pt_df_delly]).reset_index().drop_duplicates(subset=['Drug', 'Variant']).set_index(['Drug', 'Variant']).sort_index()
