...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run the BRATB pipeline samplesheet validation')
    ...
    standard_bratb_fields = ['Study', 'Sample', 'Library', 'Attempt', 'R1', 'R2', 'Flowcell', 'Lane', 'Index Sequence']
    ...
    with open(args['input_file'], 'r') as infile, open(args['output_file'], 'w', newline='') as outfile:
        ...
        writer = csv.DictWriter(outfile,
                                fieldnames=standard_bratb_fields + ['bratb_sample_name', 'bratb_bam_rg_string'],
        ...
        for row in reader:
            ...
            # Create the bratb_sample_name column
            row[
                'bratb_sample_name'] = f"{row['Study']}.{row['Sample']}.L{row['Library']}.A{row['Attempt']}.{row['Flowcell']}.{row['Lane']}.{row['Index Sequence']}"

            # Create the bratb_bwa_rg_string column
            row[
                'bratb_bam_rg_string'] = f"@RG\\tID:{row['Flowcell']}.{row['Lane']}\\tSM:{row['Study']}.{row['Sample']}\\tPL:illumina\\tLB:lib{row['Library']}\\tPU:{row['Flowcell']}.{row['Lane']}.{row['Index Sequence']}"
        ...
        with open('bratb_samplesheet.json', 'w') as f:
