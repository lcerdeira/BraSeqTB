...
conda_tools    = "bioconda::braseqtb-gather=1.0.4"
...
process GATHER {
    ...
    container "${ workflow.containerEngine == 'singularity' && !params.singularity_pull_docker_container ?
        'https://depot.galaxyproject.org/singularity/braseqtb-gather:1.0.4--hdfd78af_0' :
        'quay.io/biocontainers/braseqtb-gather:1.0.4--hdfd78af_0' }"
    ...
    if [ "${runtype}" == "paired-end" ]; then
        ...
        if [ "${task.attempt}" == "${params.max_retry}" ]; then
            echo "Unable to download ${prefix} from both SRA and ENA ${params.max_retry} times. This may or may 
                not be a temporary connection issue. Rather than stop the whole braseqtb run, 
